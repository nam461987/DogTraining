...
app.controller("ListController",
    ...
    function ($scope, popupService, $window, $location, Config, svcCache, svcQueue, Model) {
        ...
        $scope.delete = function (id, status) {
            ...
            $scope.applySuccess = Model.DeleteVideo({
                obj: obj
            }, function (response) {
        ...
        $scope.reload = function () {
                    ...
                    switch (k) {
                        case "Keyword":
                            if ($scope.condition[k].length > 3) {
                                obj._c["Name"] = "PATINDEX('%{0}%',Name) > 0 OR PATINDEX('%{0}%',Description) > 0".format($scope.condition[k]);
                            }
                            break;
                        default:
            ...
            Model.GetAllVideo({ obj: obj }, function (response) {
                if (response.Result == 1) {
...
app.controller("CreateController",
    ...
    function ($scope, $location, svcCache, svcQueue, Config, Model, Upload) {
        ...
        $scope.ok = function () {
            ...
            $scope.applySuccess = Model.InsertVideo({
                obj: obj
            }, function (response) {
                if (response.Result == 1) {
                    $location.path('/');
...
app.controller("UpdateController",
    ...
    function ($scope, $stateParams, svcCache, svcQueue, $location, Config, Model, Upload) {
        ...
        $scope.ok = function () {
            ...
            $scope.applySuccess = Model.UpdateVideo({
                obj: obj
            }, function (response) {
                if (response.Result == 1) {
                    $location.path('/');
        ...
        //Records
        Model.GetVideoById({ id: $stateParams.id }, function (response) {
            if (response.Result == 1) {
