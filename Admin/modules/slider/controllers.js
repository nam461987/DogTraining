...
app.controller("ListController",
    ...
    function ($scope, popupService, $window, $location, Config, svcCache, svcQueue, Model) {
        ...
        $scope.delete = function (id, status) {
            ...
            $scope.applySuccess = Model.DeleteSlider({
                obj: obj
            }, function (response) {
        ...
        $scope.reload = function () {
                    ...
                    switch (k) {
                        case "Keyword":
                            if ($scope.condition[k].length > 3) {
                                obj._c["Title"] = "PATINDEX('%{0}%',Title) > 0 OR PATINDEX('%{0}%',Description) > 0".format($scope.condition[k]);
                            }
                            break;
                        default:
            ...
            Model.GetAllSlider({ obj: obj }, function (response) {
                if (response.Result == 1) {
...
app.controller("UpdateController",
    ...
    function ($scope, $stateParams, svcCache, svcQueue, $location, Config, Model, Upload) {
        ...
        $scope.uploadFiles = function (files, errFiles) {
            ...
            angular.forEach(files, function (file) {
                ...
                file.upload.then(function (response) {
                    $scope.record.Image = response.data;
                }, function (response) {
        ...
        $scope.ok = function () {
            ...
            $scope.applySuccess = Model.UpdateSlider({
                obj: obj
            }, function (response) {
                if (response.Result == 1) {
                    $location.path('/');
        ...
        //Records
        Model.GetSliderById({ id: $stateParams.id }, function (response) {
            if (response.Result == 1) {
